...
public class PackAddValueByHandworkMacroHandler implements IMacroTokenHandler {
	 ...
	 */
		...
		helper.addAttribute(IAtomEngineContextConstant.ATTR_ACK_ADDVALUE_HANDWORK_LIST,atomFunction.getObjectId());//手工打包体列表
	...
	private void checkFieldAndValue(Map<Object, Object> context,IMacroToken token,String[] params,String[] fields){
		...
		if(params.length>0 && params.length >fields.length){
			message ="行:"+token.getLineNo()+MacroConstant.PACK_ADDVALUE_HANDWORK_MACRONAME+"宏"+"字段值数量:"+params.length+"大于"+MacroConstant.PACK_ADDFIELD_HANDWORK_MACRONAME+"宏字段数量:"+fields.length;
