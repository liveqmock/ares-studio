...
import com.hundsun.ares.studio.atom.compiler.token.PROCResultSetReturnToken;
...
public class PROCResultSetReturnMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		 ...
		 */
		 ...
		 String cursorName = this.getCursorName(context);//游标名称
		 String []sqlFields =this.getSqlFields(context);//取得字段列表
		 codeList.add(new PROCResultSetReturnToken(cursorName,sqlFields) );//添加CodeToken
	 ...
	 * @return 取行字段名称
	 */
	private String[]getSqlFields(Map<Object, Object> context){
		ITokenDomain domain = getDomain(context);
		return (String[])domain.getArgs()[1];
	}
