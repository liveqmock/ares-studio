...
public class FunctionEndToken implements ICodeToken {
	...
	public String genCode(Map<Object, Object> context) throws Exception {
		...
		Set<String> dbConns =helper.getAttribute(IAtomEngineContextConstant.ATTR_DATABASE_CONN_VARIABLE_LIST);
		if(dbConns.size()>0){
	...
	public static String genEndCodes(Map<Object, Object> context,String free_conn_str) throws Exception {
		...
		if(obj != null && obj instanceof AtomFunction){
			...
			//pack变量释放
			Set<String> packList = helper.getAttribute(IAtomEngineContextConstant.ATTR_PACK_VALIABLESET);
			...
			Set<String> nrsprocedureCallList = helper.getAttribute(IAtomEngineContextConstant.ATTR_PROCEDURE_CALL_RSRETURN);
	...
	private static String AddSerEndConditon(AtomFunction func, IARESProject project, Map<Object, Object> context) throws Exception {
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IAtomEngineContextConstant.SKELETON_ATTRIBUTE_HELPER);
		Set<String> procSet = helper.getAttribute(IAtomEngineContextConstant.ATTR_PROC_MACRO);
		...
		if(func.isOutputCollection()){
			Set<String> rsSet = helper.getAttribute(IAtomEngineContextConstant.ATTR_FUNC_RESULTSET);
			Set<String> packList = helper.getAttribute(IAtomEngineContextConstant.ATTR_ACK_ADDVALUE_HANDWORK_LIST);//手工打包体列表
			...
			for(Parameter  param : outputParameters){
				...
				//if(!AtomFunctionCompilerUtil.isParameterINInputParameterByName(func, id,project)){
					if(param.getParamType().getValue()==ParamType.OBJECT_VALUE){
						...
						String type = getRealType(param,project,MetadataServiceProvider.C_TYPE);
	...
	private static String getRealType(Parameter param,IARESProject project,String type){
		return AtomFunctionCompilerUtil.getRealDataType(param.getId(), project, type);
	}
	...
	private static String   getObjectDefine(Map<Object, Object> context){
		...
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IAtomEngineContextConstant.SKELETON_ATTRIBUTE_HELPER);
		Set<String> initInObjectList = helper.getAttribute(IAtomEngineContextConstant.ATTR_IN_OBJECT_INIT_VARIABLE_LIST);
		Set<String> noInInitObjectList = helper.getAttribute(IAtomEngineContextConstant.ATTR_IN_OBJECT_NOINIT_VARIABLE_LIST);
