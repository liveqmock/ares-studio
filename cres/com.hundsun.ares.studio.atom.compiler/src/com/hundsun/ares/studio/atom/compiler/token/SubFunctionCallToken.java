...
//	private final static String IP_PAKER_ADDVALUE_FLOAT = "lpFuncInPacker->AddDouble(%1$s);\t//%2$s";
	...
	public String genCode(Map<Object, Object> context) throws Exception {
		...
		callRes = (AtomFunction)context.get(IAtomEngineContextConstant.ResourceModel);//调用资源，有可能是af/as
		...
		List<String> pararms = (List<String>) context.get(IAtomEngineContextConstant.PseudoCode_Para_LIST);
		return String.format(SUB_FUNC_CALL_RETURN_CODE, atomFuc.getChineseName(),genInputParamentPackAddField(ipParas,opParas,defaultValue,context),
				genInputParamentPackAddValue(ipParas, opParas,pararms , defaultValue),
	...
	private String resultSetGetValue(Map<Object, Object> context,List<Parameter> opParas,Map<String, String> defaultValue) throws Exception {
		...
		for (Parameter parameter : opParas) {
			...
			Set<String> rsList = (Set<String>)helper.getAttribute(IAtomEngineContextConstant.ATTR_PROC_VARIABLE_LIST);
...
//	}
	...
	private String getFirstRowFlagE(Map<Object, Object> context) {
		if(token.getFlag().contains(MarkConfig.MARK_IFLAG_E)) {
			ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IAtomEngineContextConstant.SKELETON_ATTRIBUTE_HELPER);
			Set<String> curVars = helper.getAttribute(IAtomEngineContextConstant.ATTR_CURSOR_LIST);
			if(curVars.size()>0){
	...
	private String genInputParamentPackAddField(List<Parameter> ipParas,List<Parameter> opParas,Map<String, String> defaultValue,Map<Object, Object> context)  throws Exception{
		...
		for (Parameter parameter : ipParas) {
			setInputParaPackAddField(ret, parameter,defaultValue);
	...
	private void setInputParaPackAddField(StringBuffer ret, Parameter parameter, Map<String, String> defaultValue)  throws Exception{
		String type = "";
		if(parameter.getParamType().getValue()!=ParamType.OBJECT_VALUE && parameter.getParamType().getValue()!=ParamType.PARAM_GROUP_VALUE){
			type = AtomFunctionCompilerUtil.getRealDataType(parameter.getId(), calledRes.getARESProject(), MetadataServiceProvider.C_TYPE);
		}
	 ...
	 * @throws Exception
	 */
	private String genInputParamentPackAddValue(List<Parameter> ipParas,List<Parameter> opParas,List<String> pararms ,Map<String, String> defaultValue)  throws Exception{
		
		StringBuffer ret = new StringBuffer();
		boolean isErrorPathInfo = false;
		//1.函数输入参数
		for (Parameter parameter : ipParas) {
			setInputParaPackAddValue(ret, defaultValue, parameter ,pararms);
			if(parameter.getId().equals("error_pathinfo")){
				isErrorPathInfo = true;
			}
		}
...
//			String realType = stdType.getValue(MetadataServiceProvider.C_TYPE);
	...
	private void setInputParaPackAddValue(StringBuffer ret, Map<String, String> defaultValue, Parameter parameter ,List<String> pararms) throws Exception{
		String paraName = parameter.getId();//参数名
		String type = "";
		try{
			if(parameter.getParamType().getValue()!=ParamType.OBJECT_VALUE && parameter.getParamType().getValue()!=ParamType.PARAM_GROUP_VALUE){
				type = AtomFunctionCompilerUtil.getRealDataType(paraName, calledRes.getARESProject(), MetadataServiceProvider.C_TYPE);
			}
		...
		if(paramNameOrValue != null) {//默认值存在
			...
			if(AtomFunctionCompilerUtil.isParameterINAtomFunctionParameterByName(callRes, paraName,calledRes.getARESProject()) || pseudoObjectParaList.contains(paraName)|| isPsoudoCode(pararms, paraName)) {
				...
				if(!beforeAddValueUseDefaultValueV2(paraName,spDftValue)) {
					try{
						if(parameter.getParamType().getValue()!=ParamType.OBJECT_VALUE && parameter.getParamType().getValue()!=ParamType.PARAM_GROUP_VALUE){
							TypeDefaultValue typeDftValue = MetadataServiceProvider.getTypeDefaultValueOfStdFieldByName(calledRes.getARESProject(), paraName);
							String dv = typeDftValue.getValue(MetadataServiceProvider.C_TYPE);
							if(TypeRule.typeRuleCharArray(type))
							{
								dv = "\"" + MarkConfig.MARK_BLANK + "\"";
							}
							spDftValue.append(AtomFunctionCompilerUtil.getTrueDefaultValueByType(type, dv,calledRes.getARESProject()));// 取中间件赋值和取值的默认值
						}
