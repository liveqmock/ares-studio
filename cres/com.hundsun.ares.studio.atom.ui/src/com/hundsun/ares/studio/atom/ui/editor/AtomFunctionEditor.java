...
import com.hundsun.ares.studio.atom.ui.editor.page.AtomFunctionInterfacePage;
...
public class AtomFunctionEditor extends EMFFormEditor {

	private InterfacePage interfacePage;
	private AtomTextEditor sourcePage;
	private AtomPreViewPage atomPreViewPage;//预览Page
	private int index;
	...
	protected void addPages() {
		interfacePage = new AtomFunctionInterfacePage(null, this, "interface", "接口");
		try {
//			addPage(new AtomFunctionBaseInfoPage(this, "baseinfo", "基本信息"));
			addPage(interfacePage);
//			addPage(new AtomFunctionIntenalVarPage(this, "internalvar", "内部变量"));
			
			// 伪代码页面
			final AtomFunction atomfunc = (AtomFunction) getInfo();
			sourcePage = new AtomTextEditor(getARESResource());
			TextBasedEditorInput input = new TextBasedEditorInput(new ITextBased() {
				@Override
				public void setText(String text) {
					Command command =  SetCommand.create(getEditingDomain(), atomfunc, AtomPackage.Literals.ATOM_FUNCTION__PSEUDO_CODE, text);
					getEditingDomain().getCommandStack().execute(command);
				}
				@Override
				public String getText() {
					return atomfunc.getPseudoCode();
				}
			}, getARESResource());
			...
			//预览页面
			addPage(atomPreViewPage=new AtomPreViewPage(this, "preview", "预览"), new TextEditorInput());
			...
			if(databaseType.equalsIgnoreCase(ProjectSettingUtil.MYSQL)){
				addPageChangedListener(new PreviewUpdater(IAtomSkeletonFactoryConstantMySQL.SKELETONTYPE_ATOM_FUNCTION_MYSQL));
			}else{
				addPageChangedListener(new PreviewUpdater(IAtomSkeletonFactoryConstant.SKELETONTYPE_ATOM_FUNCTION));
			}
