...
public class LogicFunctionCallMacroHandler implements  IMacroTokenHandler {
	...
	/*	if(logic.isOutputCollection()){
		...
		//被调用的加入到结果集变量声明中
		helper.addAttribute(ILogicEngineContextConstant.ATTR_FUNC_RESULTSET,StringUtils.isBlank(logicFunction.getObjectId())?logicFunction.getName():logicFunction.getObjectId());
