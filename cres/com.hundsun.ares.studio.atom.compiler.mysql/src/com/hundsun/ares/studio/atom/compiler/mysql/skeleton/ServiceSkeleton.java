...
import com.hundsun.ares.studio.atom.compiler.mysql.token.service.ServiceBeginToken;
import com.hundsun.ares.studio.atom.compiler.mysql.token.service.ServiceDatabaseConnectionBeginToken;
import com.hundsun.ares.studio.atom.compiler.mysql.token.service.ServiceDatabaseConnectionEndToken;
import com.hundsun.ares.studio.atom.compiler.mysql.token.service.ServiceEndToken;
...
public class ServiceSkeleton extends BussinessSkeleton {
	...
	public Iterator<ICodeToken> getSkeletonToken() throws Exception {
		...
		AtomService func = (AtomService)context.get(IAtomEngineContextConstantMySQL.ResourceModel);
	...
	public IMacroTokenHandler getMacroTokenHandler(IMacroToken macro,
		...
		if(macroType < 0 && context.containsKey(IAtomEngineContextConstantMySQL.UserMacro_Service)){
			...
			if(fmservice.isUserMacro(macro.getKeyword())){
				...
				AtomService service = (AtomService)context.get(IAtomEngineContextConstantMySQL.ResourceModel);
				IARESProject project = (IARESProject)context.get(IAtomEngineContextConstantMySQL.Aresproject);
				businessType.putAll(AtomFunctionCompilerUtil.getParamterBusinessType2Map(service.getInputParameters(), project,context));
				businessType.putAll(AtomFunctionCompilerUtil.getParamterBusinessType2Map(service.getOutputParameters(), project,context));
