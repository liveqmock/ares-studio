...
import com.hundsun.ares.studio.atom.compiler.mysql.constant.DomainConstantMySQL;
import com.hundsun.ares.studio.atom.compiler.mysql.constant.IAtomEngineContextConstantMySQL;
import com.hundsun.ares.studio.atom.compiler.mysql.macro.TokenDomain;
import com.hundsun.ares.studio.atom.compiler.mysql.token.SubFunctionCallToken;
...
public class FunctionCallMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_FUNC_CALL,getKey());
		...
		if(!StringUtils.contains(token.getFlag(), "N")){
			...
			helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_GETLAST_RESULTSET,objectid);
		}
		boolean returnResultSet = af.isOutputCollection();
		//if(returnResultSet){
		//由于如果返回结果集，调用返回结果集的AF以及[通用SELECT]都可以直接打包输出
		helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_FUNC_RESULTSET,objectid);
		//}
				...
				if (valueVarName.indexOf("@") >= 0) {// 如果默认参数值为变量
					...
					if(parameter!=null && parameter.getParamType().getValue()!=ParamType.OBJECT_VALUE){
						popVarList.add(procVarName);
							...
							helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_IN_OBJECT_NOINIT_VARIABLE_LIST,paramKey);
						}
					}
				}
			}
	...
	private void addDomain(Map<Object, Object> context,String objectId){
		...
		String []args = new String[1];
		 args[0] = objectId;
		handler.addDomain(new TokenDomain(DomainConstantMySQL.FUNC_RESULT_OBJECT_RETURN_DOMAIN,args));
	}
