...
import com.hundsun.ares.studio.atom.compiler.mysql.token.NestPackAddFieldToken;
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		if (token.getParameters().length !=2) {
			...
			if(StringUtils.indexOf(parentPackName,"@")>-1){
				...
				if(!StringUtils.endsWith(parentPackName,"ResultSet")){
			...
			helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_OUT_OBJECT_INIT_VARIABLE_LIST, packName);//加入到要声明的对象输入列表中
