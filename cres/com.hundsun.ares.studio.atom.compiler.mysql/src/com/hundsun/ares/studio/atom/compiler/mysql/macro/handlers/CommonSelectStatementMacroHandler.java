...
import com.hundsun.ares.studio.atom.compiler.mysql.token.CommonSelectStatementToken;
...
public class CommonSelectStatementMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_FUNC_RESULTSET,rsID);
		helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_RESULTSET_LIST,rsID);//所需释放的结果集列表
		helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_GETLAST_RESULTSET,rsID);//取得就近的结果集Id，凡是可以用lpResultSet->取就近结果集的，都需要加到这个列表中
		//[通用SELECT]为数据库宏
		helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_DATABASE_MACRO,getKey());
