...
*/
package com.hundsun.ares.studio.atom.compiler.mysql.macro.procedure;

import java.util.HashMap;
import java.util.Map;

import com.hundsun.ares.studio.atom.compiler.mysql.macro.handlers.ProcedureCallMacroHandler;
...
public class ProcedureMacroTokenService implements IProcedureMacroTokenService {
	...
	private void loadProcedure(String procedureName){
		if(!prcMap.containsKey(procedureName)){
			ReferenceInfo refInfo =ReferenceManager.getInstance().getFirstReferenceInfo(project, IProcedureRefType.PROCEDURE_CNAME, procedureName, true);
			if(refInfo!=null){
				Procedure prc = (Procedure) refInfo.getObject();
				if(prc!=null){
					prcResourceMap.put(prc.getChineseName(), refInfo.getResource());
					prcMap.put(prc.getChineseName(), new ProcedureCallMacroHandler(prc.getChineseName(), refInfo.getResource()));
				}
			}
		}
	}
