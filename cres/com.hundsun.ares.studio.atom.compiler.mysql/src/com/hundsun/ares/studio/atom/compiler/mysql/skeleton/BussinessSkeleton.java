...
import com.hundsun.ares.studio.atom.compiler.mysql.macro.filter.DefaultMacroTokenFilter;
import com.hundsun.ares.studio.atom.compiler.mysql.macro.func.FunctionMacroTokenService;
import com.hundsun.ares.studio.atom.compiler.mysql.macro.procedure.ProcedureMacroTokenService;
import com.hundsun.ares.studio.atom.compiler.mysql.skeleton.util.DefaultSkeletonAttributeHelper;
import com.hundsun.ares.studio.atom.compiler.mysql.skeleton.util.ParamDefineHelper;
import com.hundsun.ares.studio.atom.compiler.mysql.skeleton.util.ParamReplaceUtil;
import com.hundsun.ares.studio.atom.compiler.mysql.token.PROCPoxyCodeToken;
import com.hundsun.ares.studio.atom.compiler.mysql.token.ResultSetProxyToken;
...
public abstract class BussinessSkeleton implements ISkeleton{
	...
	public BussinessSkeleton(IARESResource resource,Map<Object, Object> context){
		...
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.Aresproject)){
			this.context.put(IAtomEngineContextConstantMySQL.Aresproject, resource.getARESProject());
		}
		...
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.MACRO_FILTER)){
			this.context.put(IAtomEngineContextConstantMySQL.MACRO_FILTER, new DefaultMacroTokenFilter(resource.getARESProject()));
		}
...
//		}
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.Function_Macro_Service)){
			this.context.put(IAtomEngineContextConstantMySQL.Function_Macro_Service, new FunctionMacroTokenService(resource.getARESProject()));
		}
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.UserMacro_Service)){
			this.context.put(IAtomEngineContextConstantMySQL.UserMacro_Service, new UserMacroTokenService(resource.getARESProject() ,false));
		}
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.Procedure_Macro_Service)){
			this.context.put(IAtomEngineContextConstantMySQL.Procedure_Macro_Service, new ProcedureMacroTokenService(resource.getARESProject()));
		}
		//全局上下文加入到运行时上下文
		...
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER)){
			ISkeletonAttributeHelper helper = new DefaultSkeletonAttributeHelper();
			this.context.put(IAtomEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER,helper );
		}
		//如果上下文中没有模型，从资源读取模型
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.ResourceModel)){
			try {
				this.context.put(IAtomEngineContextConstantMySQL.ResourceModel, resource.getInfo(EObject.class));
			} catch (ARESModelException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}
		if(!this.context.containsKey(IAtomEngineContextConstantMySQL.PARAM_DEFINE_HELPER)){
			this.context.put(IAtomEngineContextConstantMySQL.PARAM_DEFINE_HELPER, new ParamDefineHelper());
		}
	}
	...
	public ICodeToken handleTextToken(ICodeToken token, Map<Object, Object> context)	throws Exception {
		...
		if((token instanceof ITextWithParamsToken) && inBlock){
			...
			for(String paramName : txtToken.getUsedParams()){
				helper.addAttribute(IAtomEngineContextConstantMySQL.ATTR_PROC_VARIABLE_LIST, paramName);
			}
			//如果在proc块当中，使用代理
	...
	public void onFinish(Map<Object, Object> context) throws Exception {
		...
		if(result.toString().indexOf("<") == -1 || result.toString().indexOf(">") == -1){
			...
			//替换所有常量
			IARESProject aresProject = (IARESProject) context.get(IAtomEngineContextConstantMySQL.Aresproject);
			if (aresProject != null) {
...
//		}
	...
	protected void validateParameter(){
		AtomFunction logic  = (AtomFunction)context.get(IAtomEngineContextConstantMySQL.ResourceModel);
