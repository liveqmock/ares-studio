...
public class PROCResultSetReturnTokenHelper {
	private String rsId;//游标名称
	private List<String> sqlFields;//打包字段
	private Map<Object, Object> context;//止下文列表
	private IMacroToken macroToken ;//当前处理的宏
	
	public PROCResultSetReturnTokenHelper(Map<Object, Object> context,String cursorName,List<String> sqlFields){
		...
		for(String fieldName : sqlFields){
			try{
				...
				if(TypeRule.typeRuleCharArray(type)){
					sb.append(define + "->" + tranTypeMethod(type) + "(trimb(@"+ fieldName + "));" + "//" + fieldName+ "\r\n");
				} else {
					sb.append(define + "->" + tranTypeMethod(type) + "(@"+ fieldName + ");" + "//" + fieldName+ "\r\n");
				}
			}
	...
	private String getFetchCursorVarString(){
		...
		for(int i=0;i<sqlFields.size();i++){
			 if(i!=sqlFields.size()-1 && sqlFields.size()>1){
				varString.append(":").append("@").append(sqlFields.get(i) + "_cur").append(",");
			}else if(i==sqlFields.size()-1){
				varString.append(":").append("@").append(sqlFields.get(i) + "_cur");
			}
			if(i%4==0){
				varString.append("\r\n");
			}
	...
	private String getMethodTemp(Map<Object, Object> context ,String type ,String fieldName){
		...
		Map<String,String> parameterInfo = getStandardFieldParameterInfo(fieldName,(IARESProject)context.get(IAtomEngineContextConstantMySQL.Aresproject));
