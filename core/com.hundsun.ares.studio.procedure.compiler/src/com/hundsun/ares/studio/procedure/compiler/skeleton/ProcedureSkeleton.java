...
*/
package com.hundsun.ares.studio.procedure.compiler.skeleton;
...
import com.hundsun.ares.studio.procedure.compiler.macro.IProcedureMacroTokenService;
import com.hundsun.ares.studio.procedure.compiler.macro.ProcedureMacroTokenService;
import com.hundsun.ares.studio.procedure.compiler.macro.filter.DefaultMacroTokenFilter;
import com.hundsun.ares.studio.procedure.compiler.skeleton.util.DefaultSkeletonAttributeHelper;
import com.hundsun.ares.studio.procedure.compiler.skeleton.util.ParamDefineHelper;
...
import com.hundsun.ares.studio.procedure.compiler.token.PROCPoxyCodeToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureAnnotationToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureBeginCodeToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureBeginToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureCreateToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureEndCodeToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureEndToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureExceptionHandlerToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureIntoHsobjectToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureParameterInitToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureParametersDefineToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureRelatedBasicDataInfoCodeToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureRelatedTableInfoCodeToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureReturnDefineToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureReturnValueToken;
import com.hundsun.ares.studio.procedure.compiler.token.ProcedureVariableDefineToken;
...
public class ProcedureSkeleton implements ISkeleton {
	...
	public ProcedureSkeleton(IARESResource resource, Map<Object, Object> context) {
		...
		if(!this.context.containsKey(IProcedureEngineContextConstant.Aresproject)){
			this.context.put(IProcedureEngineContextConstant.Aresproject, resource.getARESProject());
		}
		...
		if(!this.context.containsKey(IProcedureEngineContextConstant.MACRO_FILTER)){
			this.context.put(IProcedureEngineContextConstant.MACRO_FILTER, new DefaultMacroTokenFilter(resource.getARESProject()));
		}

		if(!this.context.containsKey(IProcedureEngineContextConstant.UserMacro_Service)){
			this.context.put(IProcedureEngineContextConstant.UserMacro_Service, new UserMacroTokenService(resource.getARESProject(),true));
		}
		if(!this.context.containsKey(IProcedureEngineContextConstant.Procedure_Macro_Service)){
			this.context.put(IProcedureEngineContextConstant.Procedure_Macro_Service, new ProcedureMacroTokenService(resource.getARESProject()));
		}
		...
		if(!this.context.containsKey(IProcedureEngineContextConstant.SKELETON_ATTRIBUTE_HELPER)){
			this.context.put(IProcedureEngineContextConstant.SKELETON_ATTRIBUTE_HELPER, new DefaultSkeletonAttributeHelper());
		}
		...
		//如果上下文中没有模型，从资源读取模型
		if(!this.context.containsKey(IProcedureEngineContextConstant.ResourceModel)){
			try {
				this.context.put(IProcedureEngineContextConstant.ResourceModel, resource.getInfo(EObject.class));
			} catch (ARESModelException e) {
				e.printStackTrace();
			}
		}
		
		if(!this.context.containsKey(IProcedureEngineContextConstant.PARAM_DEFINE_HELPER)){
			this.context.put(IProcedureEngineContextConstant.PARAM_DEFINE_HELPER, new ParamDefineHelper());
		}
	...
	public Iterator<ICodeToken> getSkeletonToken() throws Exception {
		...
		Procedure procedure = (Procedure)getRuntimeContext().get(IProcedureEngineContextConstant.ResourceModel);
		
		//是否需要出错处理-返回出错信息
		boolean isNotReturnErrorInfo = 
			(Boolean) getRuntimeContext().get(IProcedureEngineContextConstant.return_error_info);
		//是否生成关联资源信息代码
		boolean isGenRelatedInfo = 
			(Boolean) getRuntimeContext().get(IProcedureEngineContextConstant.gen_related_info);
		//是否生成前置代码
		boolean isGenBeginCode = 
			(Boolean) getRuntimeContext().get(IProcedureEngineContextConstant.gen_begin_code);
		//是否生成后置代码
		boolean isGenEndCode = 
			(Boolean) getRuntimeContext().get(IProcedureEngineContextConstant.gen_end_code);
...
//		}
		...
		if((token instanceof ITextWithParamsToken) && inBlock){
			...
			for(String paramName : txtToken.getUsedParams()){
				helper.addAttribute(IProcedureEngineContextConstant.ATTR_PROC_VARIABLE_LIST, paramName);
			}
		}
...
//		}
		...
		//存储过程宏处理
		if(context.containsKey(IProcedureEngineContextConstant.Procedure_Macro_Service)){
			IProcedureMacroTokenService prcservice = (IProcedureMacroTokenService)context.get(IProcedureEngineContextConstant.Procedure_Macro_Service);
		...
		//用户自定义宏处理
		if(macroType < 0 && context.containsKey(IProcedureEngineContextConstant.UserMacro_Service)){
			IUserMacroTokenService fmservice = (IUserMacroTokenService)context.get(IProcedureEngineContextConstant.UserMacro_Service);
			if(fmservice.isUserMacro(macro.getKeyword())){
				macroType = 3;
				Map<String,String> businessType = new HashMap<String, String>();
				Procedure pro = (Procedure)getRuntimeContext().get(IProcedureEngineContextConstant.ResourceModel);
				IARESProject project = (IARESProject)getRuntimeContext().get(IProcedureEngineContextConstant.Aresproject);
				...
				//给handler注入游标
				ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IProcedureEngineContextConstant.SKELETON_ATTRIBUTE_HELPER);
				Set<String> cursorList = helper.getAttribute(IProcedureEngineContextConstant.ATTR_CURSOR_LIST);
				Set<String> rsIdSet = helper.getAttribute(IProcedureEngineContextConstant.ATTR_GETLAST_RESULTSET);
				String lastId = "";
				if(rsIdSet.size() > 0){
		...
		IMacroTokenFilter filter = (IMacroTokenFilter)context.get(IProcedureEngineContextConstant.MACRO_FILTER);
