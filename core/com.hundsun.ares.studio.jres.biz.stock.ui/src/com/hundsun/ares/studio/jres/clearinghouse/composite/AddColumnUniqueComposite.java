 ...
 *
 */
public class AddColumnUniqueComposite extends ModifyActionColumnComposite<ModifyDetail>{

	public AddColumnUniqueComposite(Composite parent, TableResourceData tableData, IARESResource resource,
			Modification action) {
	...
	protected EReference getEReference() {
		return ChousePackage.Literals.ADD_TABLE_COLUMN_UNIQUE_MODIFYCATION__DETAILS;
	}
	...
	protected EList<ModifyDetail> getActionItems(Modification modification) {
		return ((AddTableColumnUniqueModifycation)action).getDetails();
	}
	...
	protected void initAction(Modification modification) {
		if(modification != null && modification instanceof AddTableColumnUniqueModifycation){
			action = modification;
		}
		else{
			action = ChouseFactory.eINSTANCE.createAddTableColumnUniqueModifycation();
		}
	}
