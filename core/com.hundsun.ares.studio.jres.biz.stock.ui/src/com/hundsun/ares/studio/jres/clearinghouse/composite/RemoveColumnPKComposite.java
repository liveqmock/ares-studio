 ...
 *
 */
public class RemoveColumnPKComposite extends ModifyActionColumnComposite<ModifyDetail>{

	public RemoveColumnPKComposite(Composite parent, TableResourceData tableData, IARESResource resource,
			Modification action) {
	...
	protected EReference getEReference() {
		return ChousePackage.Literals.REMOVE_TABLE_COLUMN_PK_MODIFICATION__DETAILS;
	}
	...
	protected EList<ModifyDetail> getActionItems(Modification modification) {
		return ((RemoveTableColumnPKModification)action).getDetails();
	}
	...
	protected void initAction(Modification modification) {
		if(modification != null && modification instanceof RemoveTableColumnPKModification){
			action = modification;
		}
		else{
			action = ChouseFactory.eINSTANCE.createRemoveTableColumnPKModification();
		}
	}
