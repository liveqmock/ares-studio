...
import com.hundsun.ares.studio.jres.clearinghouse.core.script.impl.TableSpaceScriptWrapImpl;
 ...
 *
 */
public class TablespaceInfoCompiler implements IResourceCompiler{

	private String functionName;
	
	public TablespaceInfoCompiler(String functionName) {
	...
	public CompilationResult compile(Object resource,
		...
		OracleSpaceResourceData info = (OracleSpaceResourceData) resource;
	...
	private IARESResource getJSResource(IARESProject project, String database) {
		...
		try {
			...
			String dir = property.getString("objectdir");
	...
	private String runScript(int modeBuilder, IARESResource jsResource,
		...
		try {
			...
			engine.eval(script);
			sql = ((Invocable) engine).invokeFunction(functionName,new TableSpaceScriptWrapImpl(res), defaultContext);
		} finally {
