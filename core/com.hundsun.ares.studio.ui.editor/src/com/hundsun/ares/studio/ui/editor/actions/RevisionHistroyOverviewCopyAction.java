...
import com.hundsun.ares.studio.ui.editor.blocks.RevisionHistoryOverviewBlock;
 ...
 * @author liaogc
 */
public class RevisionHistroyOverviewCopyAction  extends Action implements IUpdateAction{
			 ...
			 * @param viewer
			 */
			public RevisionHistroyOverviewCopyAction(ColumnViewer viewer) {
				super();
				this.viewer = viewer;
			...
			protected List<Object> getSelectedObjects() {
				...
				if (selection != null && selection instanceof IStructuredSelection) {
					...
					for (Object obj : ((IStructuredSelection) selection).toList()) {
						if (obj instanceof RevisionHistoryOverviewBlock.RevisionHistoryOverviewElement) {
								...
								for (Object object : selectedObjects) {
									for (int i = 0; i < columnCount; i++) {
										...
										if (lp instanceof IColumnViewerTextCopySupport) {
											sb.append(escape(StringUtils.defaultString(((IColumnViewerTextCopySupport) lp).getCopyText(object))));
										} else {
											sb.append(escape(StringUtils.defaultString( lp.getText(object))));
										}
									}
									sb.append("\n");
								}
								List<EObject> selectedRevisionHistroies = new ArrayList<EObject>();
								for(Object element:selectedObjects){
									EObject revisionHistroy = ((RevisionHistoryOverviewBlock.RevisionHistoryOverviewElement)element).getRevision();
									selectedRevisionHistroies.add(revisionHistroy);
								}
								ARESEMFClipboard.getInstance().copyToClipboard(selectedRevisionHistroies.toArray(new EObject[selectedRevisionHistroies.size()]), sb.toString(), null);
							} catch (Exception e) {
								e.printStackTrace();
							} finally {
								monitor.done();
