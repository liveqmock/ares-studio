...
public class ProcedureParametersDefineToken implements ICodeToken {
	...
	private static String COMMENT="-- ";//注解,MySQL中用-- 注意有一个空格
	private final static String PARAM_DEFINE = "\t%2$s p_%1$s %3$s%4$s";
	...
	public String genCode(Map<Object, Object> context) throws Exception {
		...
		ProcedureCompilerUtil.getOutTotleParameters(project ,this.procedure.getOutputParameters() ,totleOutputParam,procedure);
		//输入参数定义
	...
	private String getParamDefineCodeStr(Parameter parameter,int type,IARESProject project) throws Exception{
		...
		//MySQL存储过程参数类型（in、out、inout）
		String inOutStr = "in";//默认in
		int flag = FLAG_IN ;
		if(StringUtils.equals(MarkConfig.MARK_IOFLAG, parameter.getFlags())){
			inOutStr = "inout ";
			if(procedure.isOutputCollection()){
				inOutStr = "in";//输出为结果集的情况下，只处理输入参数，不处理输出参数
			}
			flag = FLAG_INOUT;
		...
		if(parameter.getParamType().getValue() == ParamType.NON_STD_FIELD_VALUE){//如果非标准字段
			...
			if(StringUtils.isNotBlank(dataType)){
				...
				//}
				BusinessDataType bizType = MetadataServiceProvider.getBusinessDataTypeByName(project, dataType);
				StandardDataType stdType = MetadataServiceProvider.getStandardDataTypeByName(project, bizType.getStdType());
				String realType = stdType.getValue(ProcedureCompilerUtil.getDatabaseType(project));
				String  length = StringUtils.defaultIfBlank(bizType.getLength(), "0");
				String precision =StringUtils.defaultIfBlank(bizType.getPrecision(),"0");
				if(StringUtils.contains(realType, "$L")){
					realType = realType.replace("$L", length);
				}
				if(StringUtils.contains(realType, "$P")){
					realType = realType.replace("$P", precision);
				}
				codeBuffer.append(String.format(PARAM_DEFINE, parameter.getId(),inOutStr,realType,StringUtils.EMPTY));
			}
			...
			if(businessDataType!=null){
				...
				String  length = StringUtils.defaultIfBlank(businessDataType.getLength(), "0");
				String precision =StringUtils.defaultIfBlank(businessDataType.getPrecision(),"0");
				if(StringUtils.contains(realType, "$L")){
					realType = realType.replace("$L", length);
				}
				if(StringUtils.contains(realType, "$P")){
				...
				if(StringUtils.isNotBlank(dataType)){
					if(StringUtils.equals(DATA_TYPE_CURSOR, realType)){//如是游标(数组情况暂时先不考虑)
						codeBuffer.append(String.format(PARAM_DEFINE, parameter.getId(),StringUtils.EMPTY,realType,StringUtils.EMPTY));
					}else{//其他情况					
						codeBuffer.append(String.format(PARAM_DEFINE, parameter.getId(),inOutStr,realType,StringUtils.EMPTY));
