...
public class ProcedureParameterInitToken implements ICodeToken {
	...
	private final static String PARAM_INIT = "\tSET v_%1$s = %2$s;" + ITokenConstant.NL;
	//输出参数前缀为P_
	private final static String EXPORT_PARAM_INIT = "\tSET p_%1$s = %2$s;" + ITokenConstant.NL;
	...
	private String initErrorPathInfo(){
		StringBuffer initCode = new StringBuffer();
		initCode.append("\tSET @error_pathinfo = substr(@error_pathinfo || '-->"+procedure.getName()+"',1,500);").append(NL);
		initCode.append("\tSET v_error_pathinfo_tmp = @error_pathinfo;").append(NL);
		return initCode.toString();
	}
