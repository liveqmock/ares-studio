...
*/
package com.hundsun.ares.studio.procedure.compiler.mysql.skeleton;
...
import com.hundsun.ares.studio.procedure.compiler.mysql.macro.IProcedureMacroTokenService;
import com.hundsun.ares.studio.procedure.compiler.mysql.macro.MacroHandlerFactory;
import com.hundsun.ares.studio.procedure.compiler.mysql.macro.ProcedureMacroTokenService;
import com.hundsun.ares.studio.procedure.compiler.mysql.macro.filter.DefaultMacroTokenFilter;
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.DefaultSkeletonAttributeHelper;
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.ParamDefineHelper;
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.ParamReplaceUtil;
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.ProcedureCompilerUtil;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.PROCPoxyCodeToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureAnnotationToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureBeginCodeToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureBeginToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureCreateToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureEndCodeToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureEndToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureExceptionHandlerToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureIntoHsobjectToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureParameterInitToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureParametersDefineToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureRelatedBasicDataInfoCodeToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureRelatedTableInfoCodeToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureReturnDefineToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureReturnValueToken;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.ProcedureVariableDefineToken;
...
public class ProcedureSkeleton implements ISkeleton {
	...
	public ProcedureSkeleton(IARESResource resource, Map<Object, Object> context) {
		...
		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.Aresproject)){
			this.context.put(IProcedureEngineContextConstantMySQL.Aresproject, resource.getARESProject());
		}
		...
		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.MACRO_FILTER)){
			this.context.put(IProcedureEngineContextConstantMySQL.MACRO_FILTER, new DefaultMacroTokenFilter(resource.getARESProject()));
		}

		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.UserMacro_Service)){
			this.context.put(IProcedureEngineContextConstantMySQL.UserMacro_Service, new UserMacroTokenService(resource.getARESProject(),true));
		}
		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.Procedure_Macro_Service)){
			this.context.put(IProcedureEngineContextConstantMySQL.Procedure_Macro_Service, new ProcedureMacroTokenService(resource.getARESProject()));
		}
		...
		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER)){
			this.context.put(IProcedureEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER, new DefaultSkeletonAttributeHelper());
		}
		...
		//如果上下文中没有模型，从资源读取模型
		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.ResourceModel)){
			try {
				this.context.put(IProcedureEngineContextConstantMySQL.ResourceModel, resource.getInfo(EObject.class));
			} catch (ARESModelException e) {
				e.printStackTrace();
			}
		}
		
		if(!this.context.containsKey(IProcedureEngineContextConstantMySQL.PARAM_DEFINE_HELPER)){
			this.context.put(IProcedureEngineContextConstantMySQL.PARAM_DEFINE_HELPER, new ParamDefineHelper());
		}
	...
	public Iterator<ICodeToken> getSkeletonToken() throws Exception {
		...
		Procedure procedure = (Procedure)getRuntimeContext().get(IProcedureEngineContextConstantMySQL.ResourceModel);
		
		//是否需要出错处理-返回出错信息
		//boolean isNotReturnErrorInfo = 
		//	(Boolean) getRuntimeContext().get(IProcedureEngineContextConstantMySQL.return_error_info);
		//是否生成关联资源信息代码
		boolean isGenRelatedInfo = 
			(Boolean) getRuntimeContext().get(IProcedureEngineContextConstantMySQL.gen_related_info);
		//是否生成前置代码
		//boolean isGenBeginCode = 
		//	(Boolean) getRuntimeContext().get(IProcedureEngineContextConstantMySQL.gen_begin_code);
		//是否生成后置代码
		//boolean isGenEndCode = 
		//	(Boolean) getRuntimeContext().get(IProcedureEngineContextConstantMySQL.gen_end_code);
...
//		}
		...
		if((token instanceof ITextWithParamsToken) && inBlock){
			...
			for(String paramName : txtToken.getUsedParams()){
				helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_PROC_VARIABLE_LIST, paramName);
			}
		}
...
//		}
		...
		//存储过程宏处理
		if(context.containsKey(IProcedureEngineContextConstantMySQL.Procedure_Macro_Service)){
			IProcedureMacroTokenService prcservice = (IProcedureMacroTokenService)context.get(IProcedureEngineContextConstantMySQL.Procedure_Macro_Service);
		...
		//用户自定义宏处理
		if(macroType < 0 && context.containsKey(IProcedureEngineContextConstantMySQL.UserMacro_Service)){
			IUserMacroTokenService fmservice = (IUserMacroTokenService)context.get(IProcedureEngineContextConstantMySQL.UserMacro_Service);
			if(fmservice.isUserMacro(macro.getKeyword())){
				macroType = 3;
				Map<String,String> businessType = new HashMap<String, String>();
				Procedure pro = (Procedure)getRuntimeContext().get(IProcedureEngineContextConstantMySQL.ResourceModel);
				IARESProject project = (IARESProject)getRuntimeContext().get(IProcedureEngineContextConstantMySQL.Aresproject);
				...
				//给handler注入游标
				ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IProcedureEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER);
				Set<String> cursorList = helper.getAttribute(IProcedureEngineContextConstantMySQL.ATTR_CURSOR_LIST);
				Set<String> rsIdSet = helper.getAttribute(IProcedureEngineContextConstantMySQL.ATTR_GETLAST_RESULTSET);
				String lastId = "";
				if(rsIdSet.size() > 0){
		...
		IMacroTokenFilter filter = (IMacroTokenFilter)context.get(IProcedureEngineContextConstantMySQL.MACRO_FILTER);
...
//			break;
	...
	public void onFinish(Map<Object, Object> context) throws Exception {
		...
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IProcedureEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER);
		Set<String> inoutParamList = helper.getAttribute(IProcedureEngineContextConstantMySQL.ATTR_IN_OUT_PARAM_LIST);
