...
import com.hundsun.ares.studio.procedure.compiler.mysql.exception.TableFieldNotFoundInStdFieldException;
import com.hundsun.ares.studio.procedure.compiler.mysql.exception.TableFieldNotSettingException;
import com.hundsun.ares.studio.procedure.compiler.mysql.exception.TableNotFoundException;
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.CodeUtil;
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.GenStringUtil;
...
public class SelectInsertTableInProcBlockToken implements ICodeToken{
	...
	public SelectInsertTableInProcBlockToken(IMacroToken macroToken,Map<Object, Object> context,Map<String, String> defaultValueMap){
		...
		this.p = (Procedure) context.get(IProcedureEngineContextConstantMySQL.ResourceModel);
	...
	private String genCodeTable()throws Exception {
		...
		for (int i = 0; i < targetColumns.size(); i++) {
			...
			StandardField tableFieldSTD  = MetadataServiceProvider.getStandardFieldByName((IARESProject)getContext().get(IProcedureEngineContextConstantMySQL.Aresproject), targerTableColumn.getFieldName());
				...
				if (defaultValueMap.get(targerTableColumn.getFieldName()) != null) {//如果字段中默认值列表中
					...
					if (sourceTableColumn != null)// 源表中存在字段
					{
						...
						if (getProcVarList().contains(targerTableColumn.getFieldName())|| getPopVarList().contains(targerTableColumn.getFieldName())||ProcedureCompilerUtil.isParameterINProcedureParameterByName(this.p, targerTableColumn.getFieldName(),project)) {
							...
							//其他情况取默认值
							TypeDefaultValue typeDefaultValue  = MetadataServiceProvider.getTypeDefaultValueOfStdFieldByName((IARESProject)getContext().get(IProcedureEngineContextConstantMySQL.Aresproject), targerTableColumn.getFieldName());
...
//						realFields.append(CodeUtil.formatInsert(targerTableColumn.getFieldName()));
	...
	private TableResourceData getTableByName(String tableName){
		...
		IARESProject aresProject = (IARESProject) this.context.get(IProcedureEngineContextConstantMySQL.Aresproject);
	...
	private Set<String> getProcVarList(){
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)getContext().get(IProcedureEngineContextConstantMySQL.SKELETON_ATTRIBUTE_HELPER);
		return  (Set<String>)helper.getAttribute(IProcedureEngineContextConstantMySQL.ATTR_PROC_VARIABLE_LIST);
	}
