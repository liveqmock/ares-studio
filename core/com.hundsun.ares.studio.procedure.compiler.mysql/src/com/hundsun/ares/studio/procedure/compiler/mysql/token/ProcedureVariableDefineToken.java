...
public class ProcedureVariableDefineToken implements ICodeToken {
	...
	private final static String PARAM_DEFINE = "\tDECLARE  v_%1$s  %2$s%3$s;";
	...
	public String genCode(Map<Object, Object> context) throws Exception {
		...
		Procedure procedure = (Procedure)context.get(IProcedureEngineContextConstantMySQL.ResourceModel);
		 ...
		 //	(Boolean) context.get(IProcedureEngineContextConstantMySQL.auto_define_input_param);
	    		...
	    		if(!parameters.contains(name)){
	    			...
	    			try{
	    				...
	    				if(null == stdField) {
	    					...
	    					//}else{
	    						declareCode.append(String.format("DECLARE v_%1$s  %2$s%3$s;", name,ProcedureCompilerUtil.getErrorVarDefaultType(name),StringUtils.EMPTY));
	    					//}
	    				}else {
		...
		if(parameter.getParamType().getValue() == ParamType.NON_STD_FIELD_VALUE){//如果非标准字段
			...
			if(businessDataType!=null){
				...
				if(StringUtils.equals(DATA_TYPE_CURSOR, realType)){//如是游标(数组情况暂时先不考虑)
					codeBuffer.append(String.format(PARAM_DEFINE, parameter.getId(),realType,StringUtils.EMPTY));
				}else{//其他情况
					//if(notDefineConnectType) {
					//	codeBuffer.append(String.format(PARAM_DEFINE, parameter.getId(),dataType,StringUtils.EMPTY));
					//}else {
	...
	private String getVarDefineCodeStr(String name,IARESProject project) throws Exception{
		...
		if(businessDataType!=null){
			...
			if(StringUtils.equals(DATA_TYPE_CURSOR, realType)){//如是游标(数组情况暂时先不考虑)
				codeBuffer.append(String.format(PARAM_DEFINE, name,realType,StringUtils.EMPTY));
			}else{//其他情况
				//if(notDefineConnectType) {
					codeBuffer.append(String.format(PARAM_DEFINE, name,realType,StringUtils.EMPTY));
				//}else{
	...
	private String defineError_pathinfo_tmp(){
		...
		try {
			...
			if(error_pathinfoEmpFieldp!=null){
				...
				if(businessDataType!=null){
					String dataType= businessDataType.getName();
					StandardDataType stdType = MetadataServiceProvider.getStandardDataTypeByName(project, businessDataType.getStdType());
					...
					//}else{
						return "\tDECLARE v_error_pathinfo_tmp  " + realType + ";  -- 错误路径临时变量\r\n";
					//}
