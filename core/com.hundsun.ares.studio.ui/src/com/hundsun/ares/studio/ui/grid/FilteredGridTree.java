...
public class FilteredGridTree  extends Composite {
	 ...
	 */
	private TreePatternFilter treePatternFilter;
	 ...
	 */
	public FilteredGridTree(Composite parent, int treeStyle, TreePatternFilter filter) {
	 ...
	 * @param parent
	 *            the parent <code>Composite</code>
	 * @see #init(int, TreePatternFilter)
	 * 
	 * @since 3.3
	 */
	protected FilteredGridTree(Composite parent) {
		super(parent, SWT.NONE);
		this.parent = parent;
	}
	 ...
	 * @since 3.3
	 */
	protected void init(int treeStyle, TreePatternFilter filter) {
		treePatternFilter = filter;
	...
	protected Control createTreeControl(Composite parent, int style) {
		...
		if (treeViewer instanceof GridTreeViewer) {
			treePatternFilter.setUseCache(true);
		}
		treeViewer.addFilter(treePatternFilter);
	...
	protected NotifyingTreeViewer doCreateTreeViewer(Composite parent, int style) {
		return new NotifyingTreeViewer(new GridEx(parent, style));
	}
	...
	private GridItem getFirstMatchingItem(GridItem[] items) {
		for (int i = 0; i < items.length; i++) {
			if (treePatternFilter.isLeafMatch(treeViewer, items[i].getData())
					&& treePatternFilter.isElementSelectable(items[i].getData())) {
				return items[i];
			}
	...
	protected WorkbenchJob doCreateRefreshJob() {
		return new WorkbenchJob("Refresh Filter") {//$NON-NLS-1$
			public IStatus runInUIThread(IProgressMonitor monitor) {
					...
					if (initial) {
						treePatternFilter.setPattern(null);
					} else if (text != null) {
						treePatternFilter.setPattern(text);
					}
					...
					try {
						...
						if (text.length() > 0 && !initial) {
							...
							int numVisibleItems = treeHeight
									/ getViewer().getGrid().getItemHeight();
						...
						GridItem[] items = getViewer().getGrid().getItems();
						if (items.length > 0
								&& getViewer().getGrid().getSelectionCount() == 0) {
							 treeViewer.refresh();
						}
			 ...
			 */
			private boolean recursiveExpand(GridItem[] items,
				...
				if (e.detail == SWT.TRAVERSE_RETURN) {
					...
					if (getViewer().getGrid().getItemCount() == 0) {
						...
						if (hasFocus && textChanged
							...
							GridItem item = getFirstMatchingItem(getViewer()
									.getGrid().getItems());
							if (item != null) {
								getViewer().getGrid().setSelection(
										new GridItem[] { item });
	 ...
	 */
	public final TreePatternFilter getPatternFilter() {
		return treePatternFilter;
	}
	 ...
	 * @return bold font
	 */
	public static Font getBoldFont(Object element, FilteredGridTree tree,
			TreePatternFilter filter) {
