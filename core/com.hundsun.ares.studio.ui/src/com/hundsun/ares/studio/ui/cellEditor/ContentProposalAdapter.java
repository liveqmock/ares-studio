...
public class ContentProposalAdapter {
	...
	class ContentProposalPopup extends PopupDialog {
		...
		protected final Control createDialogArea(final Composite parent) {
			...
			// 2012-2-22 sundl 支持 StyledString，这样可以用不同的颜色区分说明信息。
			TableOwnerDrawSupport.install(proposalTable);
		...
		private void handleSetData(Event event) {
			...
			if (0 <= index && index < proposals.length) {
				...
				// 2012-2-22 sundl 增加扩展接口，如果content proposal实现了这个接口就可以使用StyledString支持，用不同的颜色显示说明信息。
				if (current instanceof IContentProposalExtension) {
					StyledString styledString = ((IContentProposalExtension) current).getStyledDisplayString();
	...
	private void openProposalPopup(boolean autoActivated) {
		if (isValid()) {
			if (popup == null) {
				...
				if ( ArrayUtils.isNotEmpty(proposals) ) {
					if (DEBUG) {
