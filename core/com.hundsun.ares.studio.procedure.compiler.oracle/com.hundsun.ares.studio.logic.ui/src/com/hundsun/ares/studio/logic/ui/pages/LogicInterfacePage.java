...
import com.hundsun.ares.studio.logic.ui.pages.block.InternalVarParameterBlock;
...
public abstract class LogicInterfacePage extends InterfacePage {
	...
	public LogicInterfacePage(EStructuralFeature interfaceFeature,
	...
	protected Section createBasicInfoSection(Composite composite,
		...
		item = createGridComposite(client,toolkit);
		Label lbUpdateTime = toolkit.createLabel(item, "更新时间：");
		...
		{
			//找出最新的版本号
			RevisionHistory his = RevisionHistoryUtil.getMaxVersionHisInfo(getEditor().getARESResource().getModule() , getResType());
			if (his != null) {
		...
		GridDataFactory.fillDefaults().grab(true, false).indent(24, 0).applyTo(objectID);
		GridDataFactory.fillDefaults().grab(true, false).indent(12, 0).applyTo(txtVersion);
		GridDataFactory.fillDefaults().grab(true, false).indent(12,0).applyTo(txtFlag);
		...
		GridDataFactory.fillDefaults().grab(true, false).indent(18, 0).applyTo(txtName);
		GridDataFactory.fillDefaults().grab(true, false).hint(10, SWT.DEFAULT).indent(24, 0).applyTo(txtCName);
		GridDataFactory.fillDefaults().grab(true, false).applyTo(txtUpdateTime);
		
		GridDataFactory.fillDefaults().grab(true, true).hint(10, 50).indent(30, 0).applyTo(txtDescription);
