...
public class LogicHyperlinkDetector implements IHyperlinkDetector {
	
	private ITextEditor editor;

	public LogicHyperlinkDetector(ITextEditor editor) {
		this.editor = editor;
	}
	...
	public IHyperlink[] detectHyperlinks(ITextViewer textViewer, IRegion region, boolean canShowMultipleHyperlinks) {
		...
		ReferenceInfo info = ReferenceManager.getInstance().getFirstReferenceInfo(project, ILogicRefType.LOGIC_SERVICE_CNAME, name, true);
		if(info == null){
			info = ReferenceManager.getInstance().getFirstReferenceInfo(project, ILogicRefType.LOGIC_FUNCTION_CNAME, name, true);
		}
		if(info == null){
			info = ReferenceManager.getInstance().getFirstReferenceInfo(project, IAtomRefType.ATOM_SERVICE_CNAME, name, true);
		}
		if(info == null){
			info = ReferenceManager.getInstance().getFirstReferenceInfo(project, IAtomRefType.ATOM_FUNCTION_CNAME, name, true);
		}
		if(info == null){
		...
		LogicHyperlink link = new LogicHyperlink(r, action);
