...
public abstract class BizImportOperation extends ImportOperation{
	...
	public void run(IProgressMonitor monitor) throws CoreException{
		...
		for (File file : files) {
			...
			ExcelParser parser = createParser(file, log);
			parser.postParseOperation = postParseOperation;
			parser.context.put("std_filed_modify_operation", stdModifyOperation);
			parser.parse();
			
			for (Module m : parser.resources.keySet()) {
				m.processName(parser.moduleNameMap, log);
			}
			
			resources.putAll(parser.resources);
