...
public class BlockWriter extends ExcelSheetWriter implements Writer {
	...
	protected void writeBlock(KeyValueBlock block) {
		...
		for (int i = 0; i < block.kvList.size(); i++) {
			...
			// 某些属性要多个属性位
			if (kv.span > 1) {
				int endcol = column + ((kv.span - 1) *2) - 1;
				CellUtil.createCell(row, endcol, StringUtils.EMPTY, getTextStyle());
				sheet.addMergedRegion(new CellRangeAddress(rowNumber, rowNumber, column - 1, endcol));
				column += ((kv.span - 1) * 2);
			}
	...
	protected void writeBlock(TextBlock block) {
		...
		fillAndMerge(row, startColumn + 2, getLabelStyle());
		...
		if (block.newRow) {
			...
			for (String line : lines) {
				...
				fillAndMerge(row, startColumn + 2, getTextStyle());
				rowNumber++;
				rows++;
			}
		} else {
	...
	private void fillAndMerge(Row row, int col, CellStyle style) {
		for (int i = 0; i < (7 - col); i++) {
			CellUtil.createCell(row, col + i, "", style);
		}
		sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), col - 1, 6));
	}
}
