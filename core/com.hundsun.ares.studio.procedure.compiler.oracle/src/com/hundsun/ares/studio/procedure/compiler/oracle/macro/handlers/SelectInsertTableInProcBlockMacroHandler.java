...
import com.hundsun.ares.studio.procedure.compiler.oracle.token.SelectInsertTableInProcBlockToken;
...
//			}
	...
	private void addPVarList(Map<String, String> defaultValueMap,
		...
		for(String paramKey: defaultValueMap.keySet()){
			...
			if (valueVarName.indexOf("@") >= 0 && !ProcedureCompilerUtil.isParameterINProcedureParameterByName(this.p, valueVarName.substring(valueVarName.indexOf("@") + 1),project)) {// 如果默认参数值为变量
				...
				((ISkeletonAttributeHelper)context.get(IProcedureEngineContextConstantOracle.SKELETON_ATTRIBUTE_HELPER)).addAttribute(IProcedureEngineContextConstantOracle.ATTR_PROC_VARIABLE_LIST,procVarName);
				helper.addAttribute(IProcedureEngineContextConstantOracle.ATTR_PROC_VARIABLE_LIST,procVarName);
	...
	private void addMacroNameToMacroList(IMacroToken token,Map<Object, Object> context){
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IProcedureEngineContextConstantOracle.SKELETON_ATTRIBUTE_HELPER);
		helper.addAttribute(IProcedureEngineContextConstantOracle.ATTR_DATABASE_MACRO,token.getKeyword());//添加到数据库列表中
	    helper.addAttribute(IProcedureEngineContextConstantOracle.ATTR_PROC_MACRO,token.getKeyword());//添加到proc宏列表中
	}
