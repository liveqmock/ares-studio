...
public class ProcedureParametersDefineToken implements ICodeToken {
	...
	public String genCode(Map<Object, Object> context) throws Exception {
		...
		// 参考宏[输入参数声明]
		//[输出参数声明]
		this.procedure = (Procedure)context.get(IProcedureEngineContextConstantOracle.ResourceModel);
		this.context = context;
		this.notDefineConnectType = (Boolean) context.get(IProcedureEngineContextConstantOracle.not_define_connect_type);
		...
		for(Parameter parameter: totleInputParam){
			if(!parameters.contains(parameter.getId())){
				...
				helper.addAttribute(IProcedureEngineContextConstantOracle.ATTR_IN_OUT_PARAM_LIST, parameter.getId());
		...
		if(procedure.isOutputCollection()) {
			...
			//游标变量
				...
				helper.addAttribute(IProcedureEngineContextConstantOracle.ATTR_IN_OUT_PARAM_LIST, "cursor");
