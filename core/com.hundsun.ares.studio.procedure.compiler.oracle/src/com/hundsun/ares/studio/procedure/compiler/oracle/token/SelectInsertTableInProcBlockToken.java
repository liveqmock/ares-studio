...
import com.hundsun.ares.studio.procedure.compiler.oracle.exception.TableFieldNotFoundInStdFieldException;
import com.hundsun.ares.studio.procedure.compiler.oracle.exception.TableFieldNotSettingException;
import com.hundsun.ares.studio.procedure.compiler.oracle.exception.TableNotFoundException;
import com.hundsun.ares.studio.procedure.compiler.oracle.skeleton.util.CodeUtil;
import com.hundsun.ares.studio.procedure.compiler.oracle.skeleton.util.GenStringUtil;
...
public class SelectInsertTableInProcBlockToken implements ICodeToken{
	...
	public SelectInsertTableInProcBlockToken(IMacroToken macroToken,Map<Object, Object> context,Map<String, String> defaultValueMap){
		...
		this.p = (Procedure) context.get(IProcedureEngineContextConstantOracle.ResourceModel);
	...
	private String genCodeTable()throws Exception {
		...
		TableResourceData targetTable = getTableByName(getTableName(targerTableName));// 根据表名，找到对应表
		TableResourceData sourceTable = getTableByName(getTableName(sourceTableName));// 根据表名，找到对应表
		if (targetTable == null) {
			insertCode.append("//插入表记录,表不存在：" + targerTableName ).append(NEWLINE);
			//如果目标表资源不在则抛出异常
			throw new TableNotFoundException(targerTableName);
		}
		if (sourceTable == null) {
			insertCode.append("//插入表记录,表不存在：" + sourceTableName ).append(NEWLINE);
			//如果源表资源不在则抛出异常
			throw new TableNotFoundException(sourceTableName);
		}
		List<TableColumn>  targetColumns = getFieldsWithoutFlag("H",targerTableName,targetTable.getColumns());//获取目标表所有列
		...
		for (int i = 0; i < targetColumns.size(); i++) {
			...
			StandardField tableFieldSTD  = MetadataServiceProvider.getStandardFieldByName((IARESProject)getContext().get(IProcedureEngineContextConstantOracle.Aresproject), targerTableColumn.getFieldName());
				...
				if (defaultValueMap.get(targerTableColumn.getFieldName()) != null) {//如果字段中默认值列表中
					...
					if (sourceTableColumn != null)// 源表中存在字段
					{
						...
						if (getProcVarList().contains(targerTableColumn.getFieldName())|| getPopVarList().contains(targerTableColumn.getFieldName())||ProcedureCompilerUtil.isParameterINProcedureParameterByName(this.p, targerTableColumn.getFieldName(),project)) {
							...
							//其他情况取默认值
							TypeDefaultValue typeDefaultValue  = MetadataServiceProvider.getTypeDefaultValueOfStdFieldByName((IARESProject)getContext().get(IProcedureEngineContextConstantOracle.Aresproject), targerTableColumn.getFieldName());
	...
	private String genCodeSelect() throws Exception {
		...
		TableResourceData targetTable =getTableByName(getTableName(targerTableName));//取得目标表资源

		if (targetTable == null) {
			insertCode.append("\t--插入表记录,表不存在：" + targerTableName + "\n");
			throw new TableNotFoundException(targerTableName);
		}
		List<TableColumn>  targetColumns =getFieldsWithoutFlag("H",targerTableName, targetTable.getColumns()) ;//获取目标表所有列
		insertCode.append("\tinsert "+"/*"+this.p.getObjectId()+"*/"+" into ");
		...
		for (int i = 0; i < targetColumns.size(); i++) {
			...
			TableColumn targerTableColumn = targetColumns.get(i);
				...
				//从用户输入的默认值中去找
               if (defaultValueMap.get(targerTableColumn.getFieldName()) != null) {//如果字段在默认值列表中
...
//						realFields.append(CodeUtil.formatInsert(targerTableColumn.getFieldName()));
	...
	private TableResourceData getTableByName(String tableName){
		...
		IARESProject aresProject = (IARESProject) this.context.get(IProcedureEngineContextConstantOracle.Aresproject);
	...
	private Set<String> getProcVarList(){
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)getContext().get(IProcedureEngineContextConstantOracle.SKELETON_ATTRIBUTE_HELPER);
		return  (Set<String>)helper.getAttribute(IProcedureEngineContextConstantOracle.ATTR_PROC_VARIABLE_LIST);
	}
