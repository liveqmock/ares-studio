...
import com.hundsun.ares.studio.atom.compiler.exception.MacroParameterDescNotDefineException;
import com.hundsun.ares.studio.atom.compiler.exception.TableFieldNotFoundInStdFieldException;
import com.hundsun.ares.studio.atom.compiler.exception.TableFieldNotSettingException;
import com.hundsun.ares.studio.atom.compiler.skeleton.util.AtomFunctionCompilerUtil;
import com.hundsun.ares.studio.atom.compiler.skeleton.util.CodeUtil;
import com.hundsun.ares.studio.atom.compiler.skeleton.util.GenStringUtil;
...
public class SelectInsertTableInProcBlockToken implements ICodeToken{
	...
	public SelectInsertTableInProcBlockToken(IMacroToken macroToken,Map<Object, Object> context,Map<String, String> defaultValueMap){
		...
		this.atomFunction = (AtomFunction) context.get(IAtomEngineContextConstant.ResourceModel);
	...
	private String genCodeTable()throws Exception {
		...
		for (int i = 0; i < targetColumns.size(); i++) {
			...
			StandardField tableFieldSTD  = MetadataServiceProvider.getStandardFieldByName((IARESProject)getContext().get(IAtomEngineContextConstant.Aresproject), targerTableColumn.getFieldName());
		...
		for (int i = 0; i < targetColumns.size(); i++) {
			...
			TableColumn targerTableColumn = targetColumns.get(i);
			String fieldDataType= AtomFunctionCompilerUtil.getRealDataType(targerTableColumn.getFieldName(), project, MetadataServiceProvider.C_TYPE);
				if (i != 0) {
					insertCode.append(",");
					if (i % 4 == 0) {//每个字段换一行
						insertCode.append(NEWLINE+"\t");
				...
				if (defaultValueMap.get(targerTableColumn.getFieldName()) != null) {//如果字段中默认值列表中
					...
					if (sourceTableColumn != null)// 源表中存在字段
					{
						...
						if (getProcVarList().contains(targerTableColumn.getFieldName())|| getProcVarList().contains(targerTableColumn.getFieldName())||AtomFunctionCompilerUtil.isParameterINAtomFunctionParameterByName(getAtomFunction(), targerTableColumn.getFieldName(),project)) {
							...
							//其他情况取默认值
							TypeDefaultValue typeDefaultValue  = MetadataServiceProvider.getTypeDefaultValueOfStdFieldByName((IARESProject)getContext().get(IAtomEngineContextConstant.Aresproject), targerTableColumn.getFieldName());
	...
	private Set<String> getProcVarList(){
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)getContext().get(IAtomEngineContextConstant.SKELETON_ATTRIBUTE_HELPER);
		return  (Set<String>)helper.getAttribute(IAtomEngineContextConstant.ATTR_PROC_VARIABLE_LIST);
	}
