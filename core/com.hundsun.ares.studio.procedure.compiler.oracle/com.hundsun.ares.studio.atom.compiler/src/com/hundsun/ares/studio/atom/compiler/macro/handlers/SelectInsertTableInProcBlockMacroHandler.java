...
import com.hundsun.ares.studio.atom.compiler.exception.MacroNotInProcBlockException;
...
import com.hundsun.ares.studio.atom.compiler.token.SelectInsertTableInProcBlockToken;
...
public class SelectInsertTableInProcBlockMacroHandler implements
	...
	private void addPVarList(Map<String, String> defaultValueMap,
		...
		for(String paramKey: defaultValueMap.keySet()){
			...
			if (valueVarName.indexOf("@") >= 0 && !AtomFunctionCompilerUtil.isParameterINAtomFunctionParameterByName(getAtomFunction(), valueVarName.substring(valueVarName.indexOf("@") + 1),project)) {// 如果默认参数值为变量
				...
				((ISkeletonAttributeHelper)context.get(IAtomEngineContextConstant.SKELETON_ATTRIBUTE_HELPER)).addAttribute(IAtomEngineContextConstant.ATTR_PROC_VARIABLE_LIST,procVarName);
	...
	private void addFieldToProc(IMacroToken macroToken,Map<Object, Object> context,Map<String, String> defaultValueMap) throws Exception{
		ISkeletonAttributeHelper helper = (ISkeletonAttributeHelper)context.get(IAtomEngineContextConstant.SKELETON_ATTRIBUTE_HELPER);
		List<String> popVarList = (List<String>)context.get(IEngineContextConstant.PSEUDO_CODE_PARA_LIST);
		Set<String> procList = (Set<String>)helper.getAttribute(IAtomEngineContextConstant.ATTR_PROC_VARIABLE_LIST);
		IARESProject project = (IARESProject) context.get(IAtomEngineContextConstant.Aresproject);
		...
		for (int i = 0; i < targetColumns.size(); i++) {
			...
			StandardField tableFieldSTD  = MetadataServiceProvider.getStandardFieldByName((IARESProject)context.get(IAtomEngineContextConstant.Aresproject), targerTableColumn.getFieldName());
