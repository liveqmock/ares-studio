...
public class PROCResultSetStatementMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		helper.addAttribute(IAtomEngineContextConstant.ATTR_CURSOR_LIST, cursorID);
		...
		codeList.add(new PROCResultSetStatementToken(token,cursorName));//添加codeToken
	...
	private void addVarList(IMacroToken token,
			Map<Object, Object> context,String []sqlField) {
		...
		for (String field : sqlField) {
			...
			helper.addAttribute(IAtomEngineContextConstant.ATTR_CURSOR_PROC_VARIABLE_LIST, field);//注意这里不需要加_cur，定义变量时，需要用到原名，所以保持原名，在变量定义时，才加_cur
			//变量替换列表，注意这里不能是helper.addAttribute(IAtomEngineContextConstant.PSEUDO_CODE_PARA_LIST, field);
			//游标变量后缀加_cur
	...
	private void addDomain(IMacroToken token,
			Map<Object, Object> context,String []sqlFields){
	...
	private String getCursorName( Map<Object, Object> context) {
		...
		if(cursorList.size()>0){
			return "cursor" + (cursorList.toArray(new String[cursorList.size()]))[cursorList.size() -1];//LinkHashSet结构，最新的在最后
		}
		return "cursor";
	}
