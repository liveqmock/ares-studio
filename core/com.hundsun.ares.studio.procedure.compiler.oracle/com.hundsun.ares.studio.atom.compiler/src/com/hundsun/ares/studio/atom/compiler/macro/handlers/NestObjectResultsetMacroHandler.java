...
import com.hundsun.ares.studio.atom.compiler.token.NestObjectResultsetToken;
...
public class NestObjectResultsetMacroHandler implements IMacroTokenHandler{
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		if (token.getParameters().length !=2) {
			...
			helper.addAttribute(IAtomEngineContextConstant.ATTR_IN_OBJECT_INIT_VARIABLE_LIST, parent);//加入到要声明的对象列表中
			helper.addAttribute(IAtomEngineContextConstant.ATTR_IN_OBJECT_NOINIT_VARIABLE_LIST, child);//加入到不需要声明的对象列表中
