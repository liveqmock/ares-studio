...
public class PROCResultSetReturnTokenHelper {
	...
	private String getFetchCursorVarString(){
		...
		for(int i=0;i<sqlFields.length;i++){
			 if(i!=sqlFields.length-1 && sqlFields.length>1){
				varString.append(":").append("@").append(sqlFields[i] + "_cur").append(",");
			}else if(i==sqlFields.length-1){
				varString.append(":").append("@").append(sqlFields[i] + "_cur");
			}
			if(i%4==0){
				varString.append("\r\n");
			}
