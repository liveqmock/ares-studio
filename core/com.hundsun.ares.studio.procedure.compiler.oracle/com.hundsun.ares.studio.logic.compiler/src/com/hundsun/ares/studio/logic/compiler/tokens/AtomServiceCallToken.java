...
import com.hundsun.ares.studio.logic.compiler.util.ModulePropertyHelper;

/**
 * @author liaogc
 *
 */
public class AtomServiceCallToken implements ICodeToken{
	private static String NL = ICodeToken.NEWLINE;
	private static String FORMAT_ADD_FIELD = "%1$s->AddField(\"%2$s\" %3$s);\r\n";
	private static String FORMAT_ADD_STR = "%1$s->AddStr(%2$s);	//%3$s \r\n";
	private static String FORMAT_GET_STR = "%1s->GetString(\"%2s\")";
	private static String FORMAT_GET_INT = "%1s->GetInt(\"%2s\")";
	private static String FORMAT_GET_DOUBLE = "%1s->GetDouble(\"%2s\")";
	private static String FORMAT_GET_CHAR = "%1s->GetChar(\"%2s\")";
	private static String FORMAT_GET_RAW = "%1s->GetRaw(\"%2s\", %3s)";
	...
	private AtomService atomService;//被调用者模型
	private Map<String, String> defaultValueList ;//默认值列表
	private  Map<Object, Object> context;//上下文环境
	private IMacroToken macroToken;//宏
	private IARESProject project;//项目
	...
	private IARESResource atomServiceResource;//被调用者资源
	
	public AtomServiceCallToken(AtomFunction logic,AtomService atomService,Map<String, String> defaultValueList,List<Parameter> inParameters ,Set<String> inList ,Set<Parameter> outList,Map<String,String> resultsetParameters,IARESResource atomServiceResource,IMacroToken macroToken,Map<Object, Object> context){
		
		this.logic = logic;
		this.atomService = atomService;
		this.defaultValueList = defaultValueList;
		this.atomServiceResource = atomServiceResource;
	...
	public String genCode(Map<Object, Object> context) throws Exception {
		...
		try{
		code.append("// 准备异步信息\r\n");
		code.append(getPrepareCode()).append(NL);
		code.append("//调用").append(atomService.getChineseName()).append(NL);
		code.append(specialParameter());//添加特别变量赋值语句
	 ...
	 * @return
	 */
	private String getPrepareCode(){
		StringBuffer prepare = new StringBuffer();
		if(this.defaultValueList.containsKey("sysnode_id")){
			String valueVarName =defaultValueList.get("sysnode_id");
			prepare.append("@sysnode_id = ").append(valueVarName).append(";").append(NL);
		}else{
			if(resultsetParameters.containsKey("sysnode_id")){
				String id = this.resultsetParameters.get("sysnode_id");
				prepare.append("@sysnode_id = lpResultSet"+id+"->GetInt"+"(\""+"sysnode_id"+"\");\r\n") ;
				
			}
		}
		try {
			String subSysId =ModulePropertyHelper.getCRESModuleSubSysId(atomServiceResource.getModule());
			if(StringUtils.isBlank(subSysId)){
				ITokenListenerManager  manager = (ITokenListenerManager)context.get(IEngineContextConstant.TOKEN_LISTENER_MANAGER);
				IARESResource resource = (IARESResource) context.get(IEngineContextConstant.CURR_RESOURCE);
				String message = "获子系统号出错:"+resource.getFullyQualifiedName();
				manager.fireEvent(new DefaultTokenEvent(ITokenConstant.EVENT_ENGINE_WARNNING,message));
			}
			prepare.append("@subsys_id = ").append(subSysId).append(";").append(NL);
		} catch (Exception e) {
		...
		prepare.append("@timeout = ").append(StringUtils.defaultIfBlank(this.atomService.getTimeout(), "5000")).append(";").append(NL);
	...
	private String questPacker(){
		...
		if(logic instanceof LogicFunction){//如果是逻辑函数
			LogicFunction logicFunction =(LogicFunction) logic; 
			if(logicFunction.isIsTransFunc()){//进行事物处理
				packerAndUnpacker.append("iReturnCode = ");
				packerAndUnpacker.append("SubServiceCallTrans(");
				packerAndUnpacker.append("lpContext,");
				packerAndUnpacker.append("lpFuncInPacker->UnPack(),");
				packerAndUnpacker.append(lpOutVarName+",");
				packerAndUnpacker.append("lpTransMonitor,");
				packerAndUnpacker.append("@timeout,");
				packerAndUnpacker.append(objectId+",");
				packerAndUnpacker.append("@branch_no,");
				packerAndUnpacker.append("@sysnode_id,");
				packerAndUnpacker.append("@subsys_id");
				packerAndUnpacker.append(");\r\n");
				
			}else{
				packerAndUnpacker.append("iReturnCode = ");
				packerAndUnpacker.append("SubServiceCall(");
				packerAndUnpacker.append("lpContext,");
				packerAndUnpacker.append("lpFuncInPacker->UnPack(),");
				packerAndUnpacker.append(lpOutVarName+",");
				packerAndUnpacker.append("@timeout,");
	...
	private String getErrorCode(){
		...
		errorCode.append("hs_strncpy(").append("@error_info,").append("lpResultSet"+atomService.getObjectId()).append("->GetStr(\"error_info\"),"+error_info_length+");").append(NL);
		errorCode.append("@error_no = iReturnCode;").append(NL);
		errorCode.append("if (").append("lpResultSet"+atomService.getObjectId()).append("->GetStr(\"error_pathinfo\") == NULL)").append(NL);
		errorCode.append("hs_strncat(@error_pathinfo,").append("\"->").append(atomService.getObjectId()).append("\"").append(","+error_pathinfo_length+");").append(NL);
		errorCode.append("else\r\n").append("hs_strncpy(@error_pathinfo, ").append("lpResultSet"+atomService.getObjectId()).append("->GetStr(\"error_pathinfo\"),"+error_pathinfo_length+");").append(NL);
	...
	private String getAddFieldValueCode(Parameter parameter,Map<String,String> parameterInfo ){
		...
		if(StringUtils.equalsIgnoreCase(parameter.getId(), "audit_action")){
			...
			return addFieldValueCode.append("lpFuncInPacker->AddChar(p_audit_action);").toString();
		}
		
		boolean isFlagm = StringUtils.indexOf(StringUtils.defaultIfBlank( atomService.getInterfaceFlag(), StringUtils.EMPTY).toLowerCase(), FLAG_M)!=-1 || StringUtils.indexOf(StringUtils.defaultIfBlank(this.macroToken.getFlag(), StringUtils.EMPTY).toLowerCase(), FLAG_M)!=-1;
		...
		if (StringUtils.isNotBlank(dataType)) {
			if (TypeRule.typeRuleInt(dataType)) {
				...
				type = TYPE_DOUBLE;
			} else if (TypeRule.typeRuleClob(dataType)||TypeRule.typeRuleObject(dataType)||parameter.getParamType().getValue()==ParamType.OBJECT_VALUE) {
				type = TYPE_RAW;
			}
		}
		...
		String paramId = StringUtils.defaultIfBlank(parameter.getId(), "");
		if(this.defaultValueList.keySet().contains(paramId)&& parameter.getParamType().getValue() != ParamType.OBJECT_VALUE){//如果在输入重新定向中
			addFieldValueCode.append(getAddFieldValueFromDefaultValueList(type,parameter,parameterInfo));
		}else if(this.resultsetParameters.keySet().contains(paramId)/*&& (logic instanceof LogicService)*/ && !BizInterfaceParameterUtil.isOutputParameterWithIO(atomService, parameter.getId(),this.project) ){//在结果集列表中
			//只有在LS中才可以用结果集变量初始化
			addFieldValueCode.append(getAddFieldValueFromResultset(type,paramId,parameterInfo));
		}else if(isCallInParametersContains(parameter)){//在调用中的输入参数中
		...
		}else if(LogicCompilerUtil.isParameterINInternalVariablesByName(logic, paramId)){//在内部变量中
			addFieldValueCode.append(getAddFieldValueFromINInternal(type,parameter,parameterInfo));
			//addPopVarList(context,parameter.getId());
		}else if(popVarList.contains(paramId)){//在伪代码中
	...
	private String getAddFieldValueFromDefaultValueList(int type,Parameter parameter,Map<String,String> parameterInfo){
		...
		if(varName.startsWith("@")&& this.resultsetParameters.keySet().contains(varName.replaceFirst("@", ""))&& (logic instanceof LogicService)){
		...
		if(TYPE_INT==type ){
			...
			if(this.isConstant(value)){
				...
				if(!StringUtils.startsWith(value, "'")&& !StringUtils.startsWith(value, "'")){//如果不存在单引号
					value = "'"+value+"'";
				}
	...
	private String getAddFieldValueFromPopVar(int type,Parameter parameter,Map<String,String> parameterInfo){
		...
		if(TYPE_INT==type ){
			...
			code = prefix+"->AddRaw"+"("+"@"+parameter.getId()+",pi_"+parameter.getId()+");";
		}
	...
	private String specialParameter(){
		StringBuffer code = new StringBuffer();
		/*if(defaultValueList.keySet().contains("sysnode_id")){
			code.append("@sysnode_id = ").append(defaultValueList.get("sysnode_id")).append(NL);
		}else if(resultsetParameters.containsKey("sysnode_id")){
			String id = this.resultsetParameters.get("sysnode_id");
			code.append("@sysnode_id = lpResultSet"+id+"->GetInt"+"(\""+"sysnode_id"+"\");\r\n") ;
