...
import com.hundsun.ares.studio.logic.compiler.tokens.LogicServiceCheckLicenseToken;
...
import com.hundsun.ares.studio.logic.compiler.tokens.LogicServicePackerReleseToken;
import com.hundsun.ares.studio.logic.compiler.tokens.LogicServiceSvrEndToken;
import com.hundsun.ares.studio.logic.compiler.tokens.LogicServiceVariableDefineToken;
 ...
 *
 */
public class LogicServiceSkeleton extends LogicBusinessSkeleton {

	/**
	 * @param resource
	 * @param context
	 */
	public LogicServiceSkeleton(IARESResource resource,
	...
	public Iterator<ICodeToken> getSkeletonToken() throws Exception {
		...
		//逻辑服务模型
		LogicService logicSer = (LogicService)getRuntimeContext().get(ILogicEngineContextConstant.ResourceModel);
		
		//添加token处理
		tList.add(new LogicServiceBeginToken());//函数创建
		// 变量声明
		tList.add(new LogicServiceVariableDefineToken());
		...
		//LF为事务函数处理开始
		tList.add(CodeTokenFactory.getInstance().createPseudoCodeToken(logicSer.getPseudoCode()));//伪代码
		//LF为事务函数处理结束
		//Svr_end处理
		tList.add(new LogicServiceSvrEndToken());
		//打包器释放处理
		tList.add(new LogicServicePackerReleseToken());
		//释放业务对象
	...
	public IMacroTokenHandler getMacroTokenHandler(IMacroToken macro,
		...
		if(macroType < 0 && context.containsKey(ILogicEngineContextConstant.UserMacro_Service)){
			...
			if(fmservice.isUserMacro(macro.getKeyword())){
				...
				List<String> params = new ArrayList<String>();
				{
					...
					LogicService lf = (LogicService) context.get(ILogicEngineContextConstant.ResourceModel);
		...
		//内部宏处理 
		if (macroType < 0) {
			IMacroTokenHandler tempHandler = MacroHandlerFactory.getInstance()
					.create(macro.getKeyword());// MacroTokenHandlerManager.getInstance().getHandler(macro.getKeyword());
			if (null != tempHandler) {
				macroType = 4;
				handler = tempHandler;
			}
