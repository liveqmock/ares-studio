...
*/
package com.hundsun.ares.studio.procedure.compiler.mysql.macro.handlers;
...
import com.hundsun.ares.studio.procedure.compiler.mysql.macro.MacroConstant;
import com.hundsun.ares.studio.procedure.compiler.mysql.token.CommonSelectStatementToken;
...
public class CommonSelectStatementMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		Set<String> rsList = helper.getAttribute(IProcedureEngineContextConstantMySQL.ATTR_RESULTSET_LIST);
		Procedure func = (Procedure)context.get(IProcedureEngineContextConstantMySQL.ResourceModel);
		int rsTotalSizeAdd1 = rsList.size() + 1;
		String rsID = func.getObjectId() + rsTotalSizeAdd1;
		//由于如果返回结果集，调用返回结果集的AF以及[通用SELECT]都可以直接打包输出
		helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_FUNC_RESULTSET,rsID);
		helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_RESULTSET_LIST,rsID);//所需释放的结果集列表
		helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_GETLAST_RESULTSET,rsID);//取得就近的结果集Id，凡是可以用lpResultSet->取就近结果集的，都需要加到这个列表中
		//[通用SELECT]为数据库宏
		helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_DATABASE_MACRO,getKey());
