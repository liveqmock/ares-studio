...
import com.hundsun.ares.studio.procedure.compiler.mysql.skeleton.util.TableResourceUtil;
...
public class InsertTableInProcBlockMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_PROC_MACRO, this.getKey());
		helper.addAttribute(IProcedureEngineContextConstantMySQL.ATTR_DATABASE_MACRO, this.getKey());
		...
		if (token.getParameters().length > 0) {
			...
			if (table == null) {
				...
				for(TableColumn tc : tableColumns){
					...
					if (std != null) {
						//加入PROC变量定义
						helper.getAttribute(IProcedureEngineContextConstantMySQL.ATTR_PROC_VARIABLE_LIST).add(tc.getFieldName());
...
//						tc.setDefaultValue(defaultValue);
	...
	private TableResourceData getTableByName(Map<Object, Object> context ,String tableName){
		IARESProject aresProject = (IARESProject) (context.get(IProcedureEngineContextConstantMySQL.Aresproject));
	...
	private class InsertTableCodeTokenImpl implements ICodeToken{
		...
		public String genCode(Map<Object, Object> context) throws Exception {
			...
			BasicResourceInfo brInfo = (BasicResourceInfo) context.get(IProcedureEngineContextConstantMySQL.ResourceModel);
