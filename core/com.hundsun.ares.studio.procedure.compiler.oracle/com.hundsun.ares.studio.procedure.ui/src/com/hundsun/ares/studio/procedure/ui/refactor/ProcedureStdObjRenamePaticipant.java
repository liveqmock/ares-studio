...
public class ProcedureStdObjRenamePaticipant extends
		AresElementRenameParicipant {
	...
	public Change createChange(IProgressMonitor pm) throws CoreException,
		...
		List<RelationInfo> relations = ReferenceManager.getInstance()
				.getRelationInfoByTarget(IBizRefType.Std_Obj, item.getName(),
						project);
		for (RelationInfo rel : relations) {
			if (resTypes.contains(rel.getHostResource().getType())) {
				changes.add(new ReferenceProviderRefChange(rel
						.getHostResource(), IBizRefType.Std_Obj,
