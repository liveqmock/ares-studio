...
import com.hundsun.ares.studio.procedure.compiler.constant.IProcedureSkeletonFactoryConstant;
...
public class GenProcedureModuleCode extends GenCresModuleCode {
	...
	private void createModuleFile(IProgressMonitor monitor,
		...
		createProcedureFile(moduleRes, "_or.sql", monitor, module, path, charset,context);
		...
		if(moduleRes.length != moduleAllRes.length) {
			...
			createProcedureFile(moduleAllRes, "_all_or.sql", monitor, module, path, charset,context);
		}
	}
		...
		if(StringUtils.isBlank(objectID)) {
			...
			if(objectID.contains(",")){
				...
				for (String objID : objIDs) {
					...
					context.put(IProcedureEngineContextConstant.ResourceModel, procedure);
					ret.append(ResourceEngin.instance.generate(
							new DefaultSkeletonInput(
									IProcedureSkeletonFactoryConstant.SKELETONTYPE_CRES_PROCEDURE,
