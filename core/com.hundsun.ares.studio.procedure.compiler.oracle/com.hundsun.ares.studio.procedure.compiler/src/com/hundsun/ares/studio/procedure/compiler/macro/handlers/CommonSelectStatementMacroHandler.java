...
import com.hundsun.ares.studio.procedure.compiler.token.CommonSelectStatementToken;
...
public class CommonSelectStatementMacroHandler implements IMacroTokenHandler {
	...
	public Iterator<ICodeToken> handle(IMacroToken token,
		...
		Set<String> rsList = helper.getAttribute(IProcedureEngineContextConstant.ATTR_RESULTSET_LIST);
		Procedure func = (Procedure)context.get(IProcedureEngineContextConstant.ResourceModel);
		int rsTotalSizeAdd1 = rsList.size() + 1;
		String rsID = func.getObjectId() + rsTotalSizeAdd1;
		//由于如果返回结果集，调用返回结果集的AF以及[通用SELECT]都可以直接打包输出
		helper.addAttribute(IProcedureEngineContextConstant.ATTR_FUNC_RESULTSET,rsID);
		helper.addAttribute(IProcedureEngineContextConstant.ATTR_RESULTSET_LIST,rsID);//所需释放的结果集列表
		helper.addAttribute(IProcedureEngineContextConstant.ATTR_GETLAST_RESULTSET,rsID);//取得就近的结果集Id，凡是可以用lpResultSet->取就近结果集的，都需要加到这个列表中
		//[通用SELECT]为数据库宏
		helper.addAttribute(IProcedureEngineContextConstant.ATTR_DATABASE_MACRO,getKey());
