	...
	protected T1 doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			...
			case BizPackage.ERROR_INFO: {
				...
				if (result == null) result = caseErrorNoItem(errorInfo);
				if (result == null) result = caseMetadataItem(errorInfo);
				if (result == null) result = caseNamedElement(errorInfo);
				if (result == null) result = caseExtensibleModel(errorInfo);
				if (result == null) result = caseIJSONData(errorInfo);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
