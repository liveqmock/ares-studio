...
public class NewMenuWizard  extends ARESResourceNewWizard {

	@Override
	protected String getResType() {
		return IMetadataResType.Menu;
	}
	...
	public void addPages() {
		...
		ARESResourceNewWizardPage page = new ARESResourceNewWizardPage("新建一个" + resDescriptor.getName(), workbench, selectedElement, getResType()){
			...
			protected void createText(Composite composite) {
				...
				//设置只读
				txt_CName.setText("菜单与功能");
				txt_CName.setEnabled(false);
				txt_Name.setText("menu");
				txt_Name.setEnabled(false);
				
			}
		};
		page.setDescription("新建一个" + resDescriptor.getName());
		getShell().setImage(AbstractUIPlugin.imageDescriptorFromPlugin(MetadataUI.PLUGIN_ID, "icons/full/obj16/menu.gif").createImage());
		addPage(page);
		addContextPage(page);
	}
}
