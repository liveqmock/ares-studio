...
public class NewStdTypeWizard  extends ARESResourceNewWizard {

	@Override
	protected String getResType() {
		return IMetadataResType.StdType;
	}
	...
	public void addPages() {
		...
		ARESResourceNewWizardPage page = new ARESResourceNewWizardPage("新建一个" + resDescriptor.getName(), workbench, selectedElement, getResType()){
			...
			protected void createText(Composite composite) {
				...
				//设置只读
				txt_CName.setText("标准数据类型");
				txt_CName.setEnabled(false);
				txt_Name.setText("defaulttype");
				txt_Name.setEnabled(false);
				
			}
		};
		page.setDescription("新建一个" + resDescriptor.getName());
		getShell().setImage(AbstractUIPlugin.imageDescriptorFromPlugin(MetadataUI.PLUGIN_ID, "icons/full/obj16/stdTypeFile.png").createImage());
		addPage(page);
		addPage(page);
		addContextPage(page);
	}
